...
## Range of Numbers
- JavaScript has a limited range for numbers. The largest and smallest reresentable numbers are.
```javaScript
...
## Special Numbers
- **`Infinity:`** Represents positive infinity. It is returned when a number exceeds the maximum limit.
```javaScript
...
```
- **`NaN`** (Not a Number) Represent unrepresentable value.
```javaScript
let result = 0 / 0;
...
```
- **`BigInt`** Although not strictly part of the Number type, the BigInt type allows for the representation of very large integers (beyond Number.MAX_SAFE_INTEGER).
```javaScript
...
## Type Conversion
- JavaScript automatically converts between strings and numbers when necessary, especially in arithmetic operations.
```javaScript
let str = "123";
